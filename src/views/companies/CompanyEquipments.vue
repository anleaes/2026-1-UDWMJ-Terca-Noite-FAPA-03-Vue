...
import { typeOf } from '../equipments/types'
...
const $q = useQuasar()
const { remove } = useCrud('/api/equipments/')
...
function goNew() {
  router.push(`/companies/${companyId.value}/equipments/new`)
}

function goEdit(id) {
  router.push(`/companies/${companyId.value}/equipments/${id}/edit`)
}
...
<template>
  <div class="eq-page">
    <HeroHeader
      ...
      :title="companyName ? `Equipamentos — ${companyName}` : 'Equipamentos'"
      subtitle="Maquinário vinculado a esta empresa"
